...
import { isAiLocalOnly, logAiLock, localLockResponse, localLockExplanation } from './server/services/aiLocalLock.js';
import { normalizeRole, classifyPerson } from './server/services/liquipediaService.js';
import { syncMatchData, buildPublicResponse, buildAdminResponse, getSettings, saveSettings, getCooldownRemaining } from './server/services/liquipediaLiveHub.js';
...
app.use((req, res, next) => {
  ...
  res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, x-admin-tools-token");
...
app.use(session({
  ...
  secret: process.env.SESSION_SECRET || 'mlbb-draft-session-secret-change-me',
  resave: false,
...
app.post("/api/draft/final-analysis", async (req, res): Promise<any> => {
  ...
  // AI LOCAL LOCK — block all remote providers in local/dev mode
  if (isAiLocalOnly()) {
    const { bluePicks: bp = [], redPicks: rp = [], blueBans: bb = [], redBans: rb = [] } = req.body;

    const heroesDirLock = path.join(DATA_DIR, "heroes");
    const loadMetaLock = (heroName: string) => {
      if (!fs.existsSync(heroesDirLock)) return null;
      const heroFiles = fs.readdirSync(heroesDirLock).filter((f) => f.endsWith(".json"));
      for (const file of heroFiles) {
        const heroData = safeJson(path.join(heroesDirLock, file), null);
    ...
    const blueHeroesLock = bp.map((name: string) => ({ name, meta: loadMetaLock(name) }));
    const redHeroesLock = rp.map((name: string) => ({ name, meta: loadMetaLock(name) }));

    const formatSummary = (heroes: { name: string; meta: any }[]) => {
...
app.post("/api/draft/ai-recommend", async (req, res): Promise<any> => {
  ...
  if (isAiLocalOnly()) {
    const { bluePicks: bp = [], redPicks: rp = [], blueBans: bb = [], redBans: rb = [] } = req.body;
    const unavailable = new Set([...bp, ...rp, ...bb, ...rb]);
    const fallbackPool = [
    ...
    logAiLock("/api/draft/ai-recommend");
    return res.json({
      recommendations: available.length > 0 ? available : [{ heroName: "N/A", role: "N/A", reason: "Semua fallback hero sudah terpakai." }],
      overallStrategy: "Local Analysis Mode - using draft database, no paid AI token used.",
      provider: "local",
      model: "local-data",
      aiLocked: true,
      costUsd: 0,
...
app.get("/api/ai/test", async (req, res) => {
  ...
  if (isAiLocalOnly()) {
    logAiLock("/api/ai/test");
    return res.json({ success: false, model: "local-data", response: "Remote AI locked in local/dev mode.", latencyMs: 0, error: "AI_LOCAL_LOCK active" });
  }
  try {
...
app.get("/api/ai/providers/test", async (req, res) => {
  if (!checkDiagnosticsAccess(req, res)) return;
  if (isAiLocalOnly()) {
    logAiLock("/api/ai/providers/test");
    return res.json({ primaryProvider: "local", fallbackProvider: "local", providers: [{ provider: "local", connected: true, model: "local-data", latencyMs: 0 }], aiLocked: true });
  }
  try {
...
app.get("/api/ai/providers/benchmark", async (req, res) => {
  if (!checkDiagnosticsAccess(req, res)) return;
  if (isAiLocalOnly()) {
    logAiLock("/api/ai/providers/benchmark");
    return res.json({ success: false, error: "Remote AI locked in local/dev mode. Set AI_ENV=production to enable benchmarks.", benchmarks: [] });
  }
  try {
    const testMsg = [
...
app.post("/api/ai/draft-analysis", async (req, res) => {
  ...
  try {
    ...
    // AI LOCAL LOCK — skip AI providers entirely in local/dev mode, return local structured analysis
    if (isAiLocalOnly()) {
      logAiLock("/api/ai/draft-analysis");
      logAIRequest({
        sessionId: (req.headers['x-session-id'] as string) || 'anonymous',
        teamBlue: blueTeam,
        teamRed: redTeam,
...
app.post("/api/ai/deep-analysis", async (req, res) => {
  // AI LOCAL LOCK — block remote providers in local/dev mode
  if (isAiLocalOnly()) {
    logAiLock("/api/ai/deep-analysis");
    return res.status(403).json({
      success: false,
      error: "Remote AI locked in local/dev mode. Set AI_ENV=production to enable deep analysis.",
      provider: "local",
      aiLocked: true,
    });
  }
...
app.post("/api/admin/liquipedia/apply-updates", async (req, res): Promise<any> => {
  ...
  try {
    ...
    // Validation before write (relaxed: allow new heroes if scraped >= local count)
    const errors: string[] = [];
    const warnings: string[] = [];
    const localCount = safeJson(HERO_STATS_FILE, []).length;
    if (scrapedHeroes.length < 80) {
      errors.push(`Too few heroes scraped: ${scrapedHeroes.length}. Minimum required: 80.`);
    } else if (scrapedHeroes.length !== localCount) {
      warnings.push(`Hero count changed: local=${localCount}, scraped=${scrapedHeroes.length}. New heroes may have been added on Liquipedia.`);
    }
    ...
    // Post-write validation (relaxed count check)
    const written = JSON.parse(fs.readFileSync(HERO_STATS_FILE, "utf8"));
    const postWriteErrors: string[] = [];
    if (!Array.isArray(written) || written.length < 80) postWriteErrors.push(`Post-write count too low: ${written?.length}`);
    ...
    try {
      ...
      try {
        await client.query(`
          CREATE TABLE IF NOT EXISTS hero_tournament_stats (
            ...
            tournament_presence TEXT DEFAULT '0',
            blue_side_picks INTEGER DEFAULT 0,
            blue_side_win   INTEGER DEFAULT 0,
            blue_side_loss  INTEGER DEFAULT 0,
            blue_side_wr    TEXT DEFAULT '0%',
            red_side_picks  INTEGER DEFAULT 0,
            red_side_win    INTEGER DEFAULT 0,
            red_side_loss   INTEGER DEFAULT 0,
            red_side_wr     TEXT DEFAULT '0%',
            bans_total      INTEGER DEFAULT 0,
            bans_presence   TEXT DEFAULT '0',
            picks_bans_total    INTEGER DEFAULT 0,
            picks_bans_presence TEXT DEFAULT '0',
            updated_at      TIMESTAMPTZ DEFAULT NOW()
          );
        `);

        for (const hero of scrapedHeroes) {
          await client.query(`
            INSERT INTO hero_tournament_stats (
              hero_name, picks_total, picks_win, picks_loss, winrate,
              tournament_presence, blue_side_picks, blue_side_win, blue_side_loss, blue_side_wr,
              red_side_picks, red_side_win, red_side_loss, red_side_wr,
              bans_total, bans_presence, picks_bans_total, picks_bans_presence, updated_at
            ) VALUES (
            ...
            ON CONFLICT (hero_name) DO UPDATE SET
              ...
              picks_win = EXCLUDED.picks_win,
              picks_loss = EXCLUDED.picks_loss,
              winrate = EXCLUDED.winrate,
              tournament_presence = EXCLUDED.tournament_presence,
              blue_side_picks = EXCLUDED.blue_side_picks,
              blue_side_win = EXCLUDED.blue_side_win,
              blue_side_loss = EXCLUDED.blue_side_loss,
              blue_side_wr = EXCLUDED.blue_side_wr,
              red_side_picks = EXCLUDED.red_side_picks,
              red_side_win = EXCLUDED.red_side_win,
              red_side_loss = EXCLUDED.red_side_loss,
              red_side_wr = EXCLUDED.red_side_wr,
              bans_total = EXCLUDED.bans_total,
              bans_presence = EXCLUDED.bans_presence,
              picks_bans_total = EXCLUDED.picks_bans_total,
              picks_bans_presence = EXCLUDED.picks_bans_presence,
              updated_at = NOW();
          `, [
            hero.hero_name,
            parseInt(hero.picks_total) || 0,
            parseInt(hero.picks_win) || 0,
            parseInt(hero.picks_loss) || 0,
            hero.winrate || '0%',
            hero.tournament_presence || '0',
            parseInt(hero.blue_side_picks) || 0,
            parseInt(hero.blue_side_win) || 0,
            parseInt(hero.blue_side_loss) || 0,
            hero.blue_side_wr || '0%',
            parseInt(hero.red_side_picks) || 0,
            parseInt(hero.red_side_win) || 0,
            parseInt(hero.red_side_loss) || 0,
            hero.red_side_wr || '0%',
            parseInt(hero.bans_total) || 0,
            hero.bans_presence || '0',
            parseInt(hero.picks_bans_total) || 0,
            hero.picks_bans_presence || '0',
          ]);
          neonWritten++;
        }
        console.log(`[Admin] Neon PostgreSQL: upserted ${neonWritten} hero stats.`);
      } finally {
        client.release();
      }
    } catch (err: any) {
      neonError = err.message;
      console.error(`[Admin] Neon PostgreSQL write failed: ${neonError}`);
    }
    ...
    res.json({
      ...
      neon: { written: neonWritten, error: neonError },
    });
  } catch (error: any) {
...
// ====================================================================
// API ROUTES: LIQUIPEDIA DATA LAYER — LOCAL SNAPSHOT
// ====================================================================

const LIQUIPEDIA_DIR = path.join(DATA_DIR, "liquipedia");
const LIQUIPEDIA_PLAYERS_FILE = path.join(LIQUIPEDIA_DIR, "players.json");
const LIQUIPEDIA_TEAMS_FILE = path.join(LIQUIPEDIA_DIR, "teams.json");
const LIQUIPEDIA_META_FILE = path.join(LIQUIPEDIA_DIR, "sync-meta.json");
const LIQUIPEDIA_PLACEMENTS_FILE = path.join(LIQUIPEDIA_DIR, "placements.json");
const LIQUIPEDIA_TRANSFERS_FILE = path.join(LIQUIPEDIA_DIR, "transfers.json");
const LIQUIPEDIA_SQUAD_FILE = path.join(LIQUIPEDIA_DIR, "squadplayers.json");
const PLAYERS_DETAIL_DIR = path.join(DATA_DIR, "players_detail");

function readLiquipediaFile(filePath: string): any[] {
  try {
...
// Ensure readable fields exist (backward compat with old snapshots)
function ensureReadablePlayer(p: any): any {
  if (!p.nicknameRaw) p.nicknameRaw = p.nickname || null;
  if (!p.teamReferenceRaw) p.teamReferenceRaw = p.teamReference || null;
  if (!p.teamReferenceReadable) p.teamReferenceReadable = p.teamReference || null;
  // Normalize roles on read (backward compat with old snapshots that have raw roles)
  if (!p.primaryRoleRaw) p.primaryRoleRaw = p.primaryRole || null;
  if (!p.secondaryRoleRaw) p.secondaryRoleRaw = p.secondaryRole || null;
  if (!p.rolesRaw) p.rolesRaw = p.roles || [];
  // Always re-normalize canonical role from raw (handles old snapshots with raw values in primaryRole)
  const canonicalPrimary = normalizeRole(p.primaryRoleRaw || p.primaryRole);
  const canonicalSecondary = normalizeRole(p.secondaryRoleRaw || p.secondaryRole);
  if (canonicalPrimary) p.primaryRole = canonicalPrimary;
  if (canonicalSecondary) p.secondaryRole = canonicalSecondary;
  // Rebuild roles array from canonical
  if (p.rolesRaw && Array.isArray(p.rolesRaw)) {
    p.roles = [...new Set(p.rolesRaw.map((r: string) => normalizeRole(r)).filter(Boolean))] as string[];
  }
  // Classify using canonical primary role
  const classification = classifyPerson(p.primaryRole || null);
  p.roleCategory = classification.category;
  p.isProPlayer = classification.isProPlayer;
  p.isTeamStaff = classification.isTeamStaff;
  p.isBroadcastTalent = classification.isBroadcastTalent;
  p.isNeedsReview = classification.isNeedsReview;
  p.classificationReason = classification.reason;
  return p;
}
function ensureReadableTeam(t: any): any {
  if (!t.nameReadable) t.nameReadable = t.name || t.pagename || "";
  if (!t.pagenameRaw) t.pagenameRaw = t.pagename || "";
  return t;
}

function readLiquipediaMeta(): any {
  try {
    if (!fs.existsSync(LIQUIPEDIA_META_FILE)) return null;
    return JSON.parse(fs.readFileSync(LIQUIPEDIA_META_FILE, "utf8"));
  } catch { return null; }
}

app.get("/api/liquipedia/status", (_req, res) => {
  const meta = readLiquipediaMeta();
  const hasPlayers = fs.existsSync(LIQUIPEDIA_PLAYERS_FILE);
  const hasTeams = fs.existsSync(LIQUIPEDIA_TEAMS_FILE);
  let playersCount = meta?.playersCount || 0;
  let teamsCount = meta?.teamsCount || 0;
  let categoryCounts = { pro_player: 0, team_staff: 0, broadcast_talent: 0, needs_review: 0 };
  if (hasPlayers) {
    const allPlayers: any[] = readLiquipediaFile(LIQUIPEDIA_PLAYERS_FILE).map(ensureReadablePlayer);
    if (!playersCount) playersCount = allPlayers.length;
    for (const p of allPlayers) {
      const cat = p.roleCategory || "needs_review";
      if (cat in categoryCounts) (categoryCounts as any)[cat]++;
      else categoryCounts.needs_review++;
    }
  }
  if (!teamsCount && hasTeams) teamsCount = readLiquipediaFile(LIQUIPEDIA_TEAMS_FILE).length;
  res.json({
    ok: true,
    sourceMode: "local-snapshot",
    hasPlayersSnapshot: hasPlayers,
    hasTeamsSnapshot: hasTeams,
    lastSyncedAt: meta?.lastSyncedAt || null,
    playersCount,
    teamsCount,
    categoryCounts,
    filtersAvailable: hasPlayers || hasTeams,
  });
});

app.get("/api/liquipedia/filters", (_req, res) => {
  try {
    const players: any[] = readLiquipediaFile(LIQUIPEDIA_PLAYERS_FILE).map(ensureReadablePlayer);
    const teams: any[] = readLiquipediaFile(LIQUIPEDIA_TEAMS_FILE).map(ensureReadableTeam);

    if (players.length === 0 && teams.length === 0) {
      return res.json({ ok: true, message: "No snapshot data. Run npm run sync:liquipedia first.", players: { nationalities: [], regions: [], statuses: [], roles: [], categories: [] }, teams: { regions: [], statuses: [] } });
    }

    const uniq = (arr: string[]) => [...new Set(arr.filter(Boolean))].sort();

    // Canonical player roles only (not raw aliases)
    const CANONICAL_PLAYER_ROLES = ["gold", "exp", "mid", "roamer", "jungler"];
    const CANONICAL_STAFF_ROLES = ["coach", "analyst", "manager"];
    const CANONICAL_TALENT_ROLES = ["caster", "commentator", "host", "content creator"];

    const playerNationalities = uniq(players.map((p) => p.nationality).filter(Boolean));
    const playerRegions = uniq(players.map((p) => p.region).filter(Boolean));
    const playerStatuses = uniq(players.map((p) => p.status).filter(Boolean));
    // Only return canonical roles that actually exist in data
    const rolesInData = new Set(players.map((p) => p.primaryRole).filter(Boolean));
    const playerRoles = [...CANONICAL_PLAYER_ROLES, ...CANONICAL_STAFF_ROLES, ...CANONICAL_TALENT_ROLES].filter((r) => rolesInData.has(r));
    const playerCategories = uniq(players.map((p) => p.roleCategory).filter(Boolean));

    const teamRegions = uniq(teams.map((t) => t.region).filter(Boolean));
    const teamStatuses = uniq(teams.map((t) => t.status).filter(Boolean));

    res.json({
      ok: true,
      players: { nationalities: playerNationalities, regions: playerRegions, statuses: playerStatuses, roles: playerRoles, categories: playerCategories },
      teams: { regions: teamRegions, statuses: teamStatuses },
    });
  } catch (error: any) {
...
app.get("/api/liquipedia/players", (req, res) => {
  try {
    if (!fs.existsSync(LIQUIPEDIA_PLAYERS_FILE)) {
      return res.json({ ok: false, error: "Liquipedia local snapshot not found. Run npm run sync:liquipedia first.", sourceMode: "local-snapshot", players: [], total: 0 });
    }

    let players: any[] = readLiquipediaFile(LIQUIPEDIA_PLAYERS_FILE).map(ensureReadablePlayer);
    const meta = readLiquipediaMeta();
    ...
    const limit = req.query.limit ? Math.min(Math.max(parseInt(String(req.query.limit), 10) || 50, 1), 500) : 50;
    const offset = req.query.offset ? Math.max(parseInt(String(req.query.offset), 10) || 0, 0) : 0;

    if (nationality) players = players.filter((p) => p.nationality && p.nationality.toLowerCase() === nationality.toLowerCase());
    if (region) players = players.filter((p) => p.region && p.region.toLowerCase() === region.toLowerCase());
    if (status) players = players.filter((p) => p.status && p.status.toLowerCase() === status.toLowerCase());
    if (role) players = players.filter((p) => p.primaryRole && p.primaryRole.toLowerCase() === role.toLowerCase());
    if (category && category !== "all") players = players.filter((p) => p.roleCategory === category);
    if (search) {
      players = players.filter((p) =>
        (p.nickname && p.nickname.toLowerCase().includes(search)) ||
        (p.nicknameRaw && p.nicknameRaw.toLowerCase().includes(search)) ||
        (p.realName && p.realName.toLowerCase().includes(search)) ||
        (p.nationality && p.nationality.toLowerCase().includes(search)) ||
        (p.teamReference && p.teamReference.toLowerCase().includes(search)) ||
        (p.teamReferenceRaw && p.teamReferenceRaw.toLowerCase().includes(search))
      );
    }

    const total = players.length;
    const sliced = players.slice(offset, offset + limit);

    res.json({ ok: true, sourceMode: "local-snapshot", lastSyncedAt: meta?.lastSyncedAt || null, players: sliced, total, limit, offset });
  } catch (error: any) {
    console.error("[Liquipedia] Players error:", error.message);
    res.status(500).json({ ok: false, error: error.message, players: [], total: 0 });
  }
});

app.get("/api/liquipedia/teams", (req, res) => {
  try {
    if (!fs.existsSync(LIQUIPEDIA_TEAMS_FILE)) {
      return res.json({ ok: false, error: "Liquipedia local snapshot not found. Run npm run sync:liquipedia first.", sourceMode: "local-snapshot", teams: [], total: 0 });
    }

    let teams: any[] = readLiquipediaFile(LIQUIPEDIA_TEAMS_FILE).map(ensureReadableTeam);
    const meta = readLiquipediaMeta();
    ...
    if (status) teams = teams.filter((t) => t.status && t.status.toLowerCase() === status.toLowerCase());
    if (region) teams = teams.filter((t) => t.region && t.region.toLowerCase() === region.toLowerCase());
    if (search) {
      teams = teams.filter((t) =>
        (t.name && t.name.toLowerCase().includes(search)) ||
        (t.nameReadable && t.nameReadable.toLowerCase().includes(search)) ||
        (t.pagename && t.pagename.toLowerCase().includes(search)) ||
        (t.pagenameRaw && t.pagenameRaw.toLowerCase().includes(search)) ||
        (t.template && t.template.toLowerCase().includes(search)) ||
        (t.region && t.region.toLowerCase().includes(search))
      );
    }

    const total = teams.length;
    const sliced = teams.slice(offset, offset + limit);

    res.json({ ok: true, sourceMode: "local-snapshot", lastSyncedAt: meta?.lastSyncedAt || null, teams: sliced, total, limit, offset });
  } catch (error: any) {
    console.error("[Liquipedia] Teams error:", error.message);
    res.status(500).json({ ok: false, error: error.message, teams: [], total: 0 });
  }
});

app.get("/api/liquipedia/player-detail", (req, res) => {
  try {
    ...
    const player = players.find((p) =>
      p.nickname === nickname || p.nicknameRaw === nickname || (p.nickname || "").toLowerCase() === nickname.toLowerCase()
    );
    if (!player) return res.status(404).json({ ok: false, error: "Player not found in local snapshot." });
    ...
    for (const t of teams) {
      ...
      if ((template && tTemplate && template === tTemplate) || (refRaw && tPagename && refRaw === tPagename) || (refReadable && tName && refReadable === tName)) {
        matchedTeam = t;
        break;
      }
    }
    ...
    // Detail data
    const allPlacements: any[] = readLiquipediaFile(LIQUIPEDIA_PLACEMENTS_FILE);
    const allTransfers: any[] = readLiquipediaFile(LIQUIPEDIA_TRANSFERS_FILE);
    const allSquad: any[] = readLiquipediaFile(LIQUIPEDIA_SQUAD_FILE);

    const playerPagename = (player.nicknameRaw || player.nickname || "").toLowerCase();
    const playerPagenameClean = playerPagename.replace(/\s+/g, "_");

    const placements = allPlacements.filter((p) => {
      const pg = (p.pagename || "").toLowerCase();
      return pg === playerPagenameClean || pg === playerPagename;
    }).slice(0, 50);

    const transfers = allTransfers.filter((t) => {
      const pg = (t.pagename || "").toLowerCase();
      return pg === playerPagenameClean || pg === playerPagename;
    }).sort((a, b) => (b.date || "").localeCompare(a.date || "")).slice(0, 30);

    const squadHistory = allSquad.filter((s) => {
      const pg = (s.pagename || "").toLowerCase();
      return pg === playerPagenameClean || pg === playerPagename;
    }).sort((a, b) => (b.datejoin || "").localeCompare(a.datejoin || "")).slice(0, 30);

    // Detail availability
    const detailDataAvailable = !!(meta?.detailData);
    const missingSections: string[] = [];
    if (!meta?.detailData?.placements?.synced) missingSections.push("placements");
    if (!meta?.detailData?.transfers?.synced) missingSections.push("transfers");
    if (!meta?.detailData?.squadplayers?.synced) missingSections.push("squadplayers");

    // Read scraped detail JSON if available
    const detailFile = path.join(PLAYERS_DETAIL_DIR, `${(nickname || "").toLowerCase()}.json`);
    let scrapedDetail: any = null;
    try {
      if (fs.existsSync(detailFile)) {
        scrapedDetail = JSON.parse(fs.readFileSync(detailFile, "utf8"));
      }
    } catch {}
    ...
    res.json({
      ...
      scrapedDetail,
      lastSyncedAt: meta?.lastSyncedAt || null,
      lastDetailSyncAt: meta?.lastDetailSyncAt || null,
    });
  } catch (error: any) {
    console.error("[Liquipedia] Player detail error:", error.message);
    res.status(500).json({ ok: false, error: error.message });
  }
});

// ====================================================================
// API ROUTES: LIQUIPEDIA LIVE MATCH HUB
// ====================================================================

// Public endpoint — returns display-safe match data
app.get("/api/liquipedia/live-hub", (req, res) => {
  try {
    const status = typeof req.query.status === "string" ? req.query.status : undefined;
    const event = typeof req.query.event === "string" ? req.query.event : undefined;
    const limit = req.query.limit ? Math.min(Math.max(parseInt(String(req.query.limit), 10) || 20, 1), 50) : undefined;
    const response = buildPublicResponse({ status, event, limit });
    res.json(response);
  } catch (err: any) {
    console.error("[LiveHub] Public error:", err.message);
    res.json({ ok: false, matches: [], events: [], warning: "Service temporarily unavailable." });
  }
});

// Admin endpoint — returns full data + settings
app.get("/api/admin/liquipedia-live-hub", (req, res) => {
  const adminEnabled = process.env.ADMIN_TOOLS_ENABLED === "true";
  if (!adminEnabled) {
    return res.status(403).json({ error: "Admin tools disabled. Set ADMIN_TOOLS_ENABLED=true in .env." });
  }
  const providedToken = req.headers["x-admin-tools-token"] as string;
  const expectedToken = process.env.ADMIN_TOOLS_ACCESS_TOKEN;
  ...
  try {
    ...
    console.error("[LiveHub] Admin error:", err.message);
    res.status(500).json({ error: "Internal error" });
  }
});

// Admin sync trigger
app.post("/api/admin/liquipedia-live-hub/sync", async (req, res) => {
  const adminEnabled = process.env.ADMIN_TOOLS_ENABLED === "true";
  if (!adminEnabled) return res.status(403).json({ error: "Admin tools disabled." });
  const providedToken = req.headers["x-admin-tools-token"] as string;
  const expectedToken = process.env.ADMIN_TOOLS_ACCESS_TOKEN;
  if (!expectedToken || providedToken !== expectedToken) return res.status(403).json({ error: "Invalid admin token." });
  ...
  try {
    ...
    if (!force) {
      ...
      if (cd > 60) {
        return res.json({ ok: false, warning: `Cooldown active. Wait ${Math.ceil(cd / 60)} minutes.`, cooldownRemainingSeconds: cd });
      }
    }
...
app.put("/api/admin/liquipedia-live-hub/settings", (req, res) => {
  const adminEnabled = process.env.ADMIN_TOOLS_ENABLED === "true";
  if (!adminEnabled) return res.status(403).json({ error: "Admin tools disabled." });
  const providedToken = req.headers["x-admin-tools-token"] as string;
  const expectedToken = process.env.ADMIN_TOOLS_ACCESS_TOKEN;
  ...
  try {
    const body = req.body || {};
    const allowed = ["publicVisible", "showCountdown", "showUpcoming", "showOngoing", "showCompleted", "mode", "selectedEventIds", "keywordFilters", "tierFilters", "maxMatches", "syncIntervalSeconds", "logoMode"];
    const update: any = {};
    ...
    // Clamp syncInterval
    if (update.syncIntervalSeconds !== undefined) {
      update.syncIntervalSeconds = Math.max(900, Math.min(10800, update.syncIntervalSeconds));
    }
    // Clamp maxMatches
    if (update.maxMatches !== undefined) {
      update.maxMatches = Math.max(1, Math.min(50, update.maxMatches));
    }
    const updated = saveSettings(update);
    res.json({ ok: true, settings: updated });
  } catch (err: any) {
    console.error("[LiveHub] Settings error:", err.message);
...
app.get('/api/db/health', (req, res) => {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(`[Liquipedia] API key configured: ${process.env.LIQUIPEDIA_API_KEY ? "YES" : "NO (endpoints will return 500)"}`);
  });
}
