...
export default function DraftSimulator({
  ...
  useEffect(() => {
    ...
    const fetchRecommendations = async () => {
      ...
      setRecsLoading(true);
      try {
        const response = await fetch(apiUrl("/api/draft/recommendation"), {
          method: "POST",
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      await saveDraft(String(user.id), draftDataStr);
      setSaveSuccess(true);
    } catch (e) {
  ...
  const dynamicPhaseLabel = (() => {
    if (isCompleted) return "Draft Complete";
    if (!currentStep) return "";
    const teamName = currentStep.side === "BLUE"
      ? (blueTeam || "Blue Team")
      : (redTeam || "Red Team");
    const action = currentStep.type === "BAN" ? "is banning" : "is picking";
  ...
  const fetchAICoach = async () => {
    ...
    try {
      ...
      const response = await fetch(apiUrl(draftMode === "mpl" ? "/api/draft/recommendation" : "/api/draft/ai-recommend"), {
        method: "POST",
        headers: {
  ...
  const evaluateDraftGame = async () => {
    ...
    try {
      ...
      const aiResponse = await fetch(apiUrl("/api/ai/draft-analysis"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        signal: controller.signal,
        body: JSON.stringify({
      ...
      const response = await fetch(apiUrl("/api/draft/final-analysis"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bluePicks,
  ...
  const totalDeckPages = Math.ceil(sortedHeroesList.length / DECK_PAGE_SIZE);
  const deckHeroes = sortedHeroesList.slice((deckPage - 1) * DECK_PAGE_SIZE, deckPage * DECK_PAGE_SIZE);

  useEffect(() => {
    setDeckPage(1);
  }, [searchQuery, roleFilter, laneFilter, tierFilter, recommendedOnly, counterOnly, synergyOnly, comfortOnly]);
  ...
  const BanSlot = ({ heroName, side }: { heroName: string; side: "blue" | "red" }) => (
    <div className={`h-11 w-11 xl:h-12 xl:w-12 aspect-square rounded-full overflow-hidden border ${side === "blue" ? "border-blue-900/50" : "border-red-900/50"} bg-slate-900/60 shrink-0`}>
      {heroName ? (
  ...
  const PickSlot = ({
    ...
    <div className={`flex items-center gap-2 h-16 xl:h-20 rounded-xl px-2.5 border transition-all
      ...
      <div className={`h-[52px] w-[52px] xl:h-[56px] xl:w-[56px] aspect-square rounded-xl shrink-0 overflow-hidden border flex items-center justify-center bg-slate-950/70
  ...
  return (
    <div className="flex flex-col gap-6">
      {!draftStarted ? (
        /* Mode Selection / Setup Screen � Redesigned */
        ...
        <div className="relative rounded-2xl overflow-hidden border border-white/[0.06] shadow-2xl bg-gradient-to-b from-[#060e1e] to-[#030810]">
          ...
          <div className="relative z-10 flex items-stretch bg-black/30 border-b border-white/[0.04]">
            ...
            <div className="flex flex-col items-center justify-center px-4 min-w-[110px] border-x border-white/[0.04] py-2">
              {!isCompleted ? (
                <>
                  <div className="text-sm font-bold text-white/90 text-center">
                    {dynamicPhaseLabel}
                  </div>
          ...
          <div className="relative z-10 flex" style={{ minHeight: "580px" }}>
            ...
            <div className="hidden xl:flex w-[280px] 2xl:w-[340px] shrink-0 flex-col border-r border-blue-900/20 bg-blue-950/5">
              ...
              <div className="px-2 pb-2 border-t border-blue-900/20 pt-2 space-y-1">
                <div className="text-[9px] text-blue-400/50 uppercase tracking-widest">Composition Health</div>
                {evaluationDashboard.teamPanels.blue.missingLanes.length > 0 ? (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {evaluationDashboard.teamPanels.blue.missingLanes.map(lane => (
                      <span key={lane} className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-red-500/20 text-red-400 border border-red-500/30">
                        {lane}
                      </span>
                    ))}
                  </div>
                ) : (
                  <div className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 mt-2">
                    Lanes Covered ✓
                  </div>
                )}
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-2 px-1">
                <span className="text-[10px] text-slate-400">
                  {sortedHeroesList.length} heroes · Page {deckPage} of {totalDeckPages}
                </span>
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => setDeckPage(p => Math.max(1, p - 1))}
                    disabled={deckPage <= 1}
                    className="px-2 py-1 rounded-lg text-[10px] font-bold text-slate-400 border border-white/[0.06] hover:border-cyan-500/20 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                  >
                    ← Prev
                  </button>
                  <button
                    onClick={() => setDeckPage(p => Math.min(totalDeckPages, p + 1))}
              ...
              {aiLoading && (
                <div className="mx-3 mb-2 p-3 rounded-xl border border-cyan-500/20 bg-cyan-500/5">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-4 h-4 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin" />
                    <span className="text-xs font-bold text-cyan-300">AI Coach Running</span>
                  </div>
                  <p className="text-[11px] text-cyan-200/70 mb-2">{aiLoadingText || "Analyzing draft..."}</p>
              ...
              {aiError && !aiLoading && (
                <div className="mx-3 mb-2 p-3 rounded-xl border border-red-500/20 bg-red-500/5">
              ...
              {displayedCoachRecommendations.length > 0 && (
                <div className="px-3 py-2 bg-black/30 border-b border-white/[0.04]">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {aiResult?.aiLocked && <span className="text-[10px] text-amber-400/80 ml-1">Local Mode</span>}
                  </div>
              ...
              <div className="flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-white/10">
                {sortedHeroesList.length > 0 ? (
                  <div className="grid grid-cols-5 sm:grid-cols-6 lg:grid-cols-7 xl:grid-cols-6 2xl:grid-cols-8 gap-1.5">
                    {deckHeroes.map(renderHeroCard)}
                  </div>
                ) : (
            ...
            <div className="hidden xl:flex w-[280px] 2xl:w-[340px] shrink-0 flex-col border-l border-red-900/20 bg-red-950/5">
              ...
              <div className="px-2 pb-2 border-t border-red-900/20 pt-2 space-y-1">
                <div className="text-[9px] text-red-400/50 uppercase tracking-widest text-right">Composition Health</div>
                {evaluationDashboard.teamPanels.red.missingLanes.length > 0 ? (
                  <div className="flex flex-wrap gap-1 mt-2 justify-end">
                    {evaluationDashboard.teamPanels.red.missingLanes.map(lane => (
                      <span key={lane} className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-red-500/20 text-red-400 border border-red-500/30">
                        {lane}
                      </span>
                    ))}
                  ...
                  <div className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 mt-2 text-right">
                    Lanes Covered ✓
                  </div>
                )}
              </div>
            ...
            <div className="p-3 min-h-[150px]">
              {analysisTab === "counter" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[300px] overflow-y-auto pr-1">
                  {evaluationDashboard.risks.blue.map((risk, i) => (
                    <div key={`blue-${i}`} className="p-3 rounded-xl border border-blue-500/30 bg-blue-500/5 text-xs">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-blue-400" />
                        <AlertTriangle className="h-3 w-3 text-blue-400 shrink-0" />
                        <span className="font-bold text-white/80">Threat</span>
                        <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded-full bg-blue-500/20 text-blue-400">Blue</span>
                      </div>
                      <p className="text-slate-400 leading-relaxed">{risk}</p>
                    </div>
                  ))}
                  {evaluationDashboard.risks.red.map((risk, i) => (
                    <div key={`red-${i}`} className="p-3 rounded-xl border border-red-500/30 bg-red-500/5 text-xs">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-red-400" />
                        <AlertTriangle className="h-3 w-3 text-red-400 shrink-0" />
                        <span className="font-bold text-white/80">Threat</span>
                        <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded-full bg-red-500/20 text-red-400">Red</span>
                      </div>
                      <p className="text-slate-400 leading-relaxed">{risk}</p>
                    </div>
                  ))}
                  {evaluationDashboard.risks.blue.length === 0 && evaluationDashboard.risks.red.length === 0 && (
                    <div className="col-span-full text-xs text-gray-600 py-3">No major threats detected yet.</div>
                  )}
                  {evaluationDashboard.recommendations.slice(0, 5).map((rec, i) => (
                    <div key={`rec-${i}`} className="p-3 rounded-xl border border-cyan-500/30 bg-cyan-500/5 text-xs">
                      <div className="flex items-center gap-2 mb-1">
              ...
              {analysisTab === "intel" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[300px] overflow-y-auto pr-1">
                  ...
                  {localTeamIntel.likelyRepicks.map((entry: any, i: number) => (
                    <div key={i} className="p-3 rounded-xl border border-indigo-500/15 bg-indigo-900/5">
                  ...
                  {localTeamIntel.likelyRebans.map((entry: any, i: number) => (
                    <div key={`reban-${i}`} className="p-3 rounded-xl border border-rose-500/15 bg-rose-900/5">
                  ...
                  {localTeamIntel.pivotCandidates.map((entry: any, i: number) => (
                    <div key={`pivot-${i}`} className="p-3 rounded-xl border border-violet-500/15 bg-violet-900/5">
                  ...
                  {localTeamIntel.similarGames.slice(0, 2).map((g: any, i: number) => (
                    <div key={`sim-${i}`} className="p-3 rounded-xl border border-slate-700/40 bg-white/[0.02]">
              ...
              <div className="p-4 space-y-4">
                ...
                {(evaluationLoading || evaluationResult) && (
                  <div className="rounded-xl border border-emerald-500/15 bg-emerald-900/5 p-4">
                    ...
                    {evaluationMeta?.aiLocked && (
                      <div className="mb-2 rounded-lg border border-amber-500/20 bg-amber-950/20 px-3 py-2 text-[11px] text-amber-300">
                        Local Analysis Mode - using draft database, no paid AI token used.
                      </div>
                    )}
                    {evaluationLoading ? (
