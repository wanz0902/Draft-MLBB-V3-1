...
import { isAiLocalOnly, logAiLock, localLockRouterResponse } from "./aiLocalLock.js";
...
export async function testProvider(providerName: AIProvider): Promise<ProviderStatus> {
  if (isAiLocalOnly() && providerName !== "local") {
    logAiLock(`testProvider(${providerName})`);
    return { provider: providerName, connected: false, latencyMs: 0, model: "blocked-by-local-lock", error: "Remote AI locked in local/dev mode" };
  }
...
export async function generateRecommendation(
  ...
  if (isAiLocalOnly()) {
    return localLockRouterResponse(buildLocalExplanation(draftData), "generateRecommendation");
  }
...
export async function analyzeDraft(
  ...
  if (isAiLocalOnly()) {
    return localLockRouterResponse(buildLocalExplanation(draftData), "analyzeDraft");
  }
...
export async function deepAnalysis(
  ...
  if (isAiLocalOnly()) {
    return localLockRouterResponse(buildLocalExplanation(draftData), "deepAnalysis");
  }
