...
Before coding any visual/design/animation task:

1. Read this file (`AGENTS.md`) for project rules.
2. Read `docs/kilo-skill-index.md` to find available skill books.
3. Read `docs/ui-ux-pro-max-skill-book.md` for design/UI UX guidance.
4. Read `docs/framer-motion-skill-book.md` for animation presets and patterns.
5. Read `reports/latest-kilo-report.md` for current project state.
6. Use Context7 only as external reference when specific API/framework docs are needed.
...
- If a referenced book/skill is missing, report it clearly instead of pretending it exists.
- Always create/update a report in `reports/`.
